...
pub mod records;
use self::records::RecordHeader;
pub use self::plugin::Plugin;
pub use self::records::MUSC;

/// Read zstrings given a length
fn read_zstring(reader: &mut Read, len: u32) -> String {
...
fn read_ident(reader: &mut Read) -> String {
...
fn read_u16(reader: &mut Read) -> u16 {
    reader.read_u16::<LittleEndian>().unwrap()
}
fn read_i32(reader: &mut Read) -> i32 {
...
fn read_u64(reader: &mut Read) -> u64 {
    reader.read_u64::<LittleEndian>().unwrap()
}
fn skip(reader: &mut Read, len: u64) {
    /*reader.fill_buf();
    reader.consume(len);*/
...
fn parse_record_header(mut reader: &mut Read) -> RecordHeader {
    RecordHeader {
...
impl Parser {
    ...
    pub fn parse(&self, p: &Path) -> Result<Plugin, Error> {
        ...
        if hedr_size != 12 {
            println!("HEDR has unusual size field ({}). Ignoring. If you get weird errors,\
        ...
        while ident.as_str() == "GRUP" {
            ...
            if group_type != 0 {
                return Err(Error::new(ErrorKind::InvalidData, "Expected top level group"));
            }
